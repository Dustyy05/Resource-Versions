...
function UIMenu.New(Title, Subtitle, RightAlign, TxtDictionary, TxtName)
    local X, Y = 0, 0
    if Title ~= nil then Title = tostring(Title) or "" else Title = "" end
    if Subtitle ~= nil then Subtitle = string.upper(Subtitle) or "" else Subtitle = "" end
    ...
    if Subtitle ~= "" and Subtitle ~= nil then
        ...
        _UIMenu.Subtitle.Text = UIResText.New(Subtitle, 8 + _UIMenu.Position.X, 110 + _UIMenu.Position.Y, 0.35, 45, 110, 185, 255, 0)
...
function UIMenu:Visible(bool)
    if bool ~= nil then
        Scully.Menus.ProcessMenus = bool
...
function MenuPool:AddSubMenu(Menu, Text, Description, RightAlign)
    if Menu() == "UIMenu" then
        local Item = UIMenuItem.New(tostring(Text), Description or "")
        Menu:AddItem(Item)
        local SubMenu = UIMenu.New(Menu.Title:Text(), Text, RightAlign)
        self:Add(SubMenu)
...
function NativeUI.CreateMenu(Title, Subtitle, RightAlign, TxtDictionary, TxtName)
    return UIMenu.New(Title, Subtitle, RightAlign, TxtDictionary, TxtName)
end
